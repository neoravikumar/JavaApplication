package com.intermediate.problemSolving;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;
import java.util.stream.Collectors;

/**
 * Equilibrium index of an array Problem Description
 * 
 * You are given an array A of integers of size N.
 * 
 * Your task is to find the equilibrium index of the given array
 * 
 * Equilibrium index of an array is an index such that the sum of elements at
 * lower indexes is equal to the sum of elements at higher indexes.
 * 
 * NOTE:
 * 
 * Array indexing starts from 0. If there is no equilibrium index then return
 * -1. If there are more than one equilibrium indexes then return the minimum
 * index.
 * 
 * 
 * 
 * Problem Constraints 1<=N<=1e5 -1e5<=A[i]<=1e5
 * 
 * 
 * Input Format First arugment contains an array A .
 * 
 * 
 * Output Format Return the equilibrium index of the given array. If no such
 * index is found then return -1.
 * 
 * 
 * Example Input Input 1: A=[-7, 1, 5, 2, -4, 3, 0] Input 2:
 * 
 * A=[1,2,3]
 * 
 * 
 * Example Output Output 1: 3 Output 2:
 * 
 * -1
 * 
 * 
 * Example Explanation Explanation 1: 3 is an equilibrium index, because: A[0] +
 * A[1] + A[2] = A[4] + A[5] + A[6] Explanation 1:
 * 
 * There is no such index.
 */
public class EquilibriumIndexArray {

	public int solve(ArrayList<Integer> A) {

		int N = null != A ? A.size() : 0;
		int result = -1;
		ArrayList<Integer> resultList = new ArrayList<Integer>();
		if (N >= 1 && N <= Math.pow(10, 5)) {
			// Calculating right sum
			int rSum = A.stream().collect(Collectors.summingInt(Integer::intValue));
			int lSum = 0;
			for (int i = 0; i < N; i++) {
				if (A.get(i) >= (-Math.pow(10, 5)) && A.get(i) <= Math.pow(10, 5)) {
					rSum -= A.get(i);
					if (lSum == rSum) {
						resultList.add(i);
					}
					lSum += A.get(i);
				}

			}
		}

		if (resultList.size() > 1) {
			int minIndex = Integer.MAX_VALUE;
			for (int a : resultList) {
				if (a < minIndex) {
					minIndex = a;
				}

			}
			result = minIndex;
		} else if (resultList.size() ==1) {
			result = resultList.get(0);
			
		} else {
			result = -1;
		}

		return result;

	}

	public static void main(String[] args) {
		//List<Integer> integers = Arrays.asList(-7, 1, 5, 0, -4, 3, 2);
		List<Integer> integers = Arrays.asList(1, 2, 3);

		ArrayList<Integer> inputArray = new ArrayList<Integer>();
		inputArray.addAll(integers);

		EquilibriumIndexArray eq = new EquilibriumIndexArray();
		int result = eq.solve(inputArray);
		System.out.println("Result:>>" + result);

	}

}
