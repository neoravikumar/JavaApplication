...
public class SubMatrixSumQueries {
	...
	public int[] solve(int[][] A, int[] B, int[] C, int[] D, int[] E) {
		...
		int n = A.length, m = A[0].length, mod = 1000000000+7;
		 ...
		 * int q = B.length;
		 * 
		 * for (int i = 0; i < q; i++) { int x1 = B[i] - 1, y1 = C[i] - 1; int x2 =
		 * D[i], y2 = E[i]; assert (x1 < x2 && y1 < y2); long data = (((dp[x2][y2]+mod)%
		 * mod) - ((dp[x1][y2]+mod)% mod) - ((dp[x2][y1]+mod)% mod) + ((dp[x1][y1]+mod)%
		 * mod))%mod; ans.add(new Long(data).intValue()); } int[] result =
		 * ans.stream().mapToInt(Integer::intValue).toArray(); return result ;
		 */
